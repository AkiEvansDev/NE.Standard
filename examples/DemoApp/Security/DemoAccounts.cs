...
/// <summary>An account the sign-in screen accepts: who, what they are, what they may do.</summary>
internal sealed record DemoAccount(string UserName, string Password, FrozenSet<string> Roles, FrozenSet<string> Permissions);

/// <summary>
/// Stands in for whatever a real application authenticates against. The screens are about what a successful lookup hands to
/// <c>UIContext.SignInAsync</c> — a user id, roles, permissions — not about how the lookup is done; the passwords sit here in
/// plain text, which is exactly what a real one must not do.
/// </summary>
internal static class DemoAccounts
{
    ...
    public static DemoAccount? Find(string? userName, string? password)
    {
        ...
        {
            if (string.Equals(account.UserName, userName.Trim(), StringComparison.OrdinalIgnoreCase) && string.Equals(account.Password, password, StringComparison.Ordinal))
                return account;
        }
