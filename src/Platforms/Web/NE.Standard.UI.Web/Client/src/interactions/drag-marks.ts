// A native drag's marks: the class a dragged element wears in the air, and the payload a drag needs to start.
// A tree's row and a tab's caption drag the same way; what a drop does with them is each engine's.

/**
 * Marks the element as the one being dragged and gives the drag its payload, clearing any stale mark first. The `companions`
 * travel with it and wear the same mark.
 */
