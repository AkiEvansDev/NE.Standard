...
/// </summary>
public abstract partial class VisualComponentBase<TComponent>(string? id = null) : IVisualComponent, IDialogOwnerComponent
    where TComponent : VisualComponentBase<TComponent>, IUIComponentDefinition
{
    ...
    private static readonly UIResponsive<UIVisibility> DefaultVisibility = UIVisibility.Visible;
    ...
    /// <inheritdoc />
    [UIComponentProperty(Contract = typeof(IVisualComponent), DefaultValueMember = nameof(DefaultVisibility))]
    public UIResponsive<UIVisibility>? Visibility { get; set; }
    ...
    /// <inheritdoc />
    public bool HasDialogs => _dialogs is { Count: > 0 };

    /// <inheritdoc />
    public IReadOnlyList<UIDialog> Dialogs => (IReadOnlyList<UIDialog>?)_dialogs ?? [];

    /// <summary>
    /// Adds a dialog of the component's own: the view renders it beside its declared ones, and the component's client opens it by key.
    /// </summary>
    public TComponent AddDialog(UIDialog dialog)
    {
    ...
    /// <summary>
    /// Sets the grid cell(s) this component occupies, optionally overriding the placement per breakpoint.
    /// </summary>
    ...
    private static void EnsurePropertiesRegistered()
    {
    ...
    IVisualComponent IVisualComponent.On(string eventName, string command, params KeyValuePair<string, UIActionArgument>[] arguments)
        => On(eventName, command, arguments);
